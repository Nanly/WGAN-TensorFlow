...
def deconv2d(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02, padding_='SAME', output_size=None,
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable('w', [k_h, k_w, k, input_shape[3]], initializer=tf.random_normal_initializer(stddev=stddev))
...
def batchnorm(x, name, _ops, is_train=True):
    """Batch normalization."""
    with tf.variable_scope(name):
...
def instance_norm(x, name='instance_norm', mean=1.0, stddev=0.02, epsilon=1e-5):
    with tf.variable_scope(name):
        depth = x.get_shape()[3]
        scale = tf.get_variable('scale', [depth], tf.float32, initializer=tf.random_normal_initializer(mean=mean, stddev=stddev, dtype=tf.float32))
