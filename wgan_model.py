...
class WGAN(object):
    ...
    def generator(self, data, name='g_'):
        with tf.variable_scope(name):
            ...
            h0_batchnorm = tf_utils.batchnorm(h0_reshape, name='h0_batchnorm', _ops=self._gen_train_ops)
            h0_relu = tf.nn.relu(h0_batchnorm, name='h0_relu')

            #8 x 8
            h1_deconv = tf_utils.deconv2d(h0_relu, self.gen_c[1], name='h1_deconv2d')
            h1_batchnorm = tf_utils.batchnorm(h1_deconv, name='h1_batchnorm', _ops=self._gen_train_ops)
            h1_relu = tf.nn.relu(h1_batchnorm, name='h1_relu')

            #16 x 16
            h2_deconv = tf_utils.deconv2d(h1_relu, self.gen_c[2], name='h2_deconv2d')
            h2_batchnorm = tf_utils.batchnorm(h2_deconv, name='h2_batchnorm', _ops=self._gen_train_ops)
            ...
            if self.flags.dataset == 'mnist':
                ...
                h3_batchnorm = tf_utils.batchnorm(h3_deconv, name='h3_batchnorm', _ops=self._gen_train_ops)
    ...
    def discriminator(self, data, name='d_', is_reuse=False):
        with tf.variable_scope(name) as scope:
            ...
            h1_batchnorm = tf_utils.batchnorm(h1_conv, name='h1_batchnorm', _ops=self._dis_train_ops)
            h1_lrelu = tf_utils.lrelu(h1_batchnorm, name='h1_lrelu')

            #16 -> 8 or 8 - >4
            h2_conv = tf_utils.conv2d(h1_lrelu, self.dis_c[2], name='h2_conv2d')
            h2_batchnorm = tf_utils.batchnorm(h2_conv, name='h2_batchnorm', _ops=self._dis_train_ops)
            ...
            if self.flags.dataset == 'mnist':
                ...
                h3_batchnorm = tf_utils.batchnorm(h3_conv, name='h3_batchnorm', _ops=self._dis_train_ops)
